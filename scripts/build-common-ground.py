...
def card(a):
    ...
    return f'''<article class="cg-story story-{e(a['id'])} {'is-lead' if a is featured else ''}" data-format="{e(a['format'])}" data-search="{e(terms.lower())}"><a class="cg-story-image" href="{e(a['url'])}" aria-label="Read {e(a['title'])}">{art(a)}</a><div class="cg-story-copy"><p class="cg-story-category">{e(a['format'])} <span>· {e(a['topics'][0])}</span></p><h2><a href="{e(a['url'])}">{e(a['title'])}</a></h2><p class="cg-story-summary">{e(a['summary'])}</p><p class="cg-story-meta">{e(a['author'])}<br><time datetime="{e(a['date'])}">{label}</time> · {e(a['readingMinutes'])} min read</p><a class="cg-story-read" href="{e(a['url'])}">Read the article <span aria-hidden="true">↗</span></a></div></article>'''
...
topic_page=topic_page.replace('<title>Common Ground | By Beyond Borders</title>','<title>Topics | Common Ground</title>').replace('<script src="common-ground.js?v=2"></script>','').replace('href="#stories" aria-current="page"','href="journal.html"')
