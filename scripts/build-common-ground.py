...
# Show subscriptions only when a real hosted signup destination is configured.
newsletter=json.loads((ROOT / 'content/common-ground.json').read_text()).get('newsletter',{})
if newsletter.get('signupUrl'):
    from urllib.parse import urlparse
    if urlparse(newsletter['signupUrl']).scheme!='https':
        raise ValueError('Newsletter signup must use HTTPS')
    signup=f'<section class="cg-subscribe"><p class="eyebrow">Stay in the conversation</p><h2>{e(newsletter["heading"])}</h2><p>{e(newsletter["description"])}</p><a href="{e(newsletter["signupUrl"])}">Subscribe by email {arrow if "arrow" in globals() else ""}</a></section>'
...
footer='<footer class="journal-footer cg-shared-footer"><a class="cg-footer-brand" href="journal.html">Common Ground<span>By Beyond Borders</span></a><nav aria-label="Footer navigation"><a href="journal.html">Latest</a><a href="common-ground-topics.html">Topics</a><a href="common-ground-standards.html">Our standards</a><a href="common-ground-contribute.html">Contribute</a><a href="index.html">Beyond Borders '+arrow+'</a></nav><p>Research commentaries interpret published work and are not independently peer reviewed.</p></footer>'
